...
export default function MultiFileUploadSubmission({
  ...
  const handlePreviewFile = (file: UploadedFile) => {
    ...
    if (data?.publicUrl) {
      // Build siblings from all uploaded files
      const siblings = uploadedFiles.map(f => {
        const { data: d } = supabase.storage.from('task-submissions').getPublicUrl(f.file_path);
        return { url: d?.publicUrl || '', name: f.file_name, size: f.file_size, filePath: f.file_path };
      });
      const activeIdx = siblings.findIndex(s => s.filePath === file.file_path);
      openPreview({
        url: data.publicUrl,
        name: file.file_name,
        size: file.file_size,
