...
export default function SubmissionHistoryPopup({ 
  ...
  // navigate removed - using FilePreviewDialog instead
  ...
  // Collect all file submissions from history for sidebar navigation
  const getAllFilesFromHistory = () => {
    const allFiles: { url: string; name: string; size?: number; filePath?: string }[] = [];
      ...
      links.forEach((link: any) => {
        if (link.file_path) {
          const { data } = supabase.storage
            ...
            .getPublicUrl(link.file_path);
          if (data?.publicUrl) {
            allFiles.push({
              url: data.publicUrl,
              name: link.file_name || link.title || 'File',
  ...
  const handleViewFile = (filePath: string, fileName: string, fileSize: number, taskTitle?: string) => {
    const { data } = supabase.storage
      .from('task-submissions')
      .getPublicUrl(filePath);
    if (data?.publicUrl) {
      const siblingFiles = getAllFilesFromHistory();
      const activeIndex = siblingFiles.findIndex(f => f.filePath === filePath);
