...
export default function FilePreview() {
  ...
  const isPublicSemanticRoute = !!(shareToken && taskSlug);
  const isDirectUrl = !!directUrl || !!resourceId;

  const handleGoBack = () => {
    if (isPublicRoute) {
      navigate(`/s/${shareToken}`);
    } else if (isSemanticRoute || resolvedProjectSlug) {
  ...
  useEffect(() => {
    ...
    } else if (isPublicSemanticRoute) {
      resolveTaskFromShareToken();
  ...
  }, [projectSlug, taskSlug, shareToken, legacyTaskId, isSemanticRoute, isPublicSemanticRoute, directUrl, resourceId]);
  ...
  const resolveTaskFromShareToken = async () => {
    if (!shareToken || !taskSlug) return;
    
    try {
      // Find group by share token (public access, no auth needed)
      ...
      // Find task by slug within group
      ...
      if (task.submission_link) {
        try {
          ...
          setError('Lỗi dữ liệu bài nộp');
          setIsLoading(false);
        }
      } else {
        setError('Task chưa có bài nộp');
        setIsLoading(false);
      }
    } catch (error) {
      console.error('Error resolving from share token:', error);
      setError('Có lỗi xảy ra');
      setIsLoading(false);
    }
  };
  ...
  const navigateToFile = (file: TaskFile, index: number) => {
    if (isPublicSemanticRoute) {
      navigate(`/s/${shareToken}/t/${taskSlug}/f/${index}`);
    } else if (isSemanticRoute) {
                ...
                <div className="flex gap-2 py-1">
                  {taskFiles.map((file, index) => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors shrink-0 max-w-[200px] ${
        ...
        {taskId && showNotesSidebar && (
          <aside className="w-full md:w-1/2 lg:w-1/3 border-l bg-background overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-4">
              <TaskNotes taskId={taskId} compact isPublicAccess={isPublicRoute} />
            </div>
          </aside>
        )}
      </div>
